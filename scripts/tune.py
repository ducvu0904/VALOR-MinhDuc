...
"""
tune.py — Optuna hyperparameter search for VALOR / RERUM models.

Usage:
    # Tune TARNet on hillstrom-men with ZILN + Focal + WR (full VALOR):
    python scripts/tune.py --model TARNet --dataset hillstrom-men \\
                   --use_ziln --use_focal --use_ranking \\
                   --n_trials 50

    # Tune DragonNet + RERUM on hillstrom-women:
    python scripts/tune.py --model DragonNet --dataset hillstrom-women \\
                   --rerum --use_uplift_ranking --use_response_ranking \\
                   --n_trials 50

Results are saved to:
    results/tuned/{dataset_tag}.csv                    — final 10-seed metrics
    results/tuned/best_params/{model}_{dataset}.json   — best hparams
"""
...
def build_trial_hparams(trial, args) -> dict:
    ...
    Search space:
    ─────────────────────────────────────────────────────────────
    Always tuned (all models):
      lr           — LogUniform(1e-5, 1e-2)
      l2_reg       — LogUniform(1e-6, 1e-2)
      hidden_dim   — Categorical([32, 64, 128])

    VALOR only  (use_focal / use_gating / use_ranking):
      focal_gamma  — DiscreteUniform(0.0, 5.0, step=0.5)
      focal_alpha  — Uniform(0.5, 0.99)

    Any ranking flag active (VALOR WR or RERUM response ranking):
      lambda_rank  — LogUniform(1e-6, 1e-1)

    RERUM uplift ranking only (--rerum --use_uplift_ranking):
      lambda_lu    — LogUniform(1e-6, 1e-1)
    """
...
def objective(trial, args, data: dict) -> float:
    ...
    and returns the mean AUUC on the VALIDATION SET.
    ...
    for seed in INNER_SEEDS:
        try:
            ...
            auuc_scores.append(metrics["auuc"])
            val_losses.append(metrics.get("val_loss", 0.0))
        except Exception as e:
            warnings.warn(f"Trial {trial.number} seed {seed} failed: {e}")
            return float("inf")

    mean_auuc = float(np.mean(auuc_scores))
    mean_val_loss = float(np.mean(val_losses))
    print(f"[Trial {trial.number}] Val Loss: {mean_val_loss:.4f} | Val AUUC: {mean_auuc:.4f}")
...
def final_evaluation(args, best_hparams: dict, data: dict) -> dict:
    ...
    Hyperparameters were selected using val AUUC; this is the held-out
...
def _get_tuned_output_path(args) -> str:
    mapping = {
        "synthetic":      "results/tuned/synthesis.csv",
        "hillstrom-men":  "results/tuned/hillstrom_men.csv",
        "hillstrom-women": "results/tuned/hillstrom_women.csv",
    }
    return mapping.get(args.dataset, f"results/tuned/{args.dataset}.csv")
...
def _get_best_params_path(args) -> str:
    ...
    return f"results/tuned/best_params/{safe_model}_{safe_dataset}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna hyperparameter tuning for VALOR / RERUM models."
    )
    ...
    study_name = args.study_name or f"{model_name}_{args.dataset}".replace(" ", "_")
    ...
    print(f"\n{'='*60}")
    print(f"  Best Trial #{best_trial.number}  |  Mean Val Loss: {best_trial.value:.4f}")
    print(f"  Best Params:")
    ...
    print(f"\n{'='*60}")
    print(f"  FINAL TUNED RESULTS: {model_name}")
    print(f"{'='*60}")
